...
public abstract class Element extends Thread {
	...
	public Element(Controller controller, int x, int y, double direction, int hitpoints){
	...
	protected void createPolygon(){
		...
		double angle = Math.tan((getHeight()/2)/(getWidth()/2));
		polygon.addPoint((int) (x + Math.cos(angle + Math.toRadians(getDirection())) * distance), 
				(int) (y + Math.sin(angle + Math.toRadians(getDirection())) * distance));
		polygon.addPoint((int) (x + Math.cos((Math.toRadians(90) - angle) + Math.toRadians(getDirection() + 90)) * distance),
				(int) (y + Math.sin((Math.toRadians(90) - angle) + Math.toRadians(getDirection() + 90)) * distance));
		polygon.addPoint((int) (x + Math.cos(angle + Math.toRadians(getDirection() + 180)) * distance),
				(int) (y + Math.sin(angle + Math.toRadians(getDirection() + 180)) * distance));
		polygon.addPoint((int) (x + Math.cos((Math.toRadians(90) - angle) + Math.toRadians(getDirection() + 270)) * distance),
				(int) (y + Math.sin((Math.toRadians(90) - angle) + Math.toRadians(getDirection() + 270)) * distance));
	}
